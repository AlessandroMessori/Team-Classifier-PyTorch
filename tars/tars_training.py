        ...
        for phase in ['train', 'val']:
            ...
            for data in tqdm(dataloaders[phase]):
               ...
               # print(labels.data)
                ...
                running_corrects += calculate_accuracy(preds, labels.data)
                # print(str(running_corrects))
...
def model_evaluation(mode, model_conv, input_data_loc, input_shape, use_gpu, name):
    ...
    dataloaders, image_datasets = data_loader_predict(
        data_loc, input_shape, name)
    ...
    for img, label in dataloaders:
        ...
        output = model_conv(inputs.view(-1, c, h, w))
        ...
        _, preds = torch.Tensor.max(outputs, 0)
    ...
    print("Accuracy_score {} : {} ".format(
        mode,  accuracy_score(original, predicted)))
...
def model_evaluation_test(mode, model_conv, test_input_data_loc, input_shape, use_gpu, name):
    dataloaders, image_datasets = data_loader_predict(
        test_input_data_loc, input_shape, name)
    imgs = [i[0] for i in image_datasets.imgs]
    print("total number of {} images: {}".format(mode, len(imgs)))
    predicted, probs = [], []
    for img, label in dataloaders:
